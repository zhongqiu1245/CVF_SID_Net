...
class BaseEditModel(BaseModel):
    ...
    def __init__(self,
        ...
        # please refer 'mmagic/models/editors/cvfsidnet/cvfsid_net.py, line 141'
        if 'restructure_in_test' in generator.keys():
            self.restructure_in_test = generator['restructure_in_test']
        else:
            self.restructure_in_test = False
    ...
    def forward(self,
        ...
        if mode == 'tensor':
            ...
            metainfo['mode'] = ['predict']
            data_samples.set_metainfo(metainfo)

            predictions = self.forward_inference(inputs, data_samples, **kwargs)
    ...
    def forward_tensor(self,
        ...
        """
        # for restructuring network in val/test, for example: repvgg
        if self.restructure_in_test:
            feats = self.generator(inputs, data_samples.metainfo['mode'][0], **kwargs)
        else:
    ...
    def forward_train(self,
        ...
        except AttributeError: # for self-supervised networks
            loss = self.pixel_loss(feats, self, data_samples)
        else: # for supervised networks, the normal one
